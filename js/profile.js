...
function renderProfile() {
  ...
  page.innerHTML = `
    <div class="page-container my-profile-page">
      ...
      <!-- ═══════ MY FOSTER LISTINGS ═══════ -->
      ${renderMyFosterSection()}
      
      <!-- ═══════ MY FOSTER APPLICATIONS ═══════ -->
      ${renderMyApplicationsSection()}
...
/* MY FOSTER LISTINGS                                            */
/* ============================================================ */
function renderMyFosterSection() {
  if (!isLoggedIn()) return '';

  const myListings = FOSTER_PETS.filter(p =>
    p.listedBy === 'user_self'
  );

  return `
    <section class="mp-section-card" id="mpFosterSection">
      <div class="mp-section-header">
        <h3><i class="fas fa-paw"></i> My Foster Listings</h3>
        <button class="mp-see-all" onclick="openAddFosterModal()">
          <i class="fas fa-plus"></i> List Pet
        </button>
      </div>

      ${!myListings.length ? `
        <div class="mp-empty-foster">
          <i class="fas fa-hand-holding-heart"></i>
          <p>You haven't listed any foster pets yet</p>
          <button class="btn btn-primary btn-sm" onclick="openAddFosterModal()">
            <i class="fas fa-plus"></i> List a Foster Pet
          </button>
        </div>
      ` : `
        <div class="mp-foster-list">
          ${myListings.map(pet => renderMyFosterCard(pet)).join('')}
        </div>
      `}
    </section>
  `;
...
function renderMyFosterCard(pet) {
  const statusColors = {
    available: { bg: 'success', label: 'Available' },
    fostered: { bg: 'info', label: 'Fostered' },
    adopted: { bg: 'purple', label: 'Adopted' }
  };
  const status = statusColors[pet.status] || statusColors.available;

  return `
    <div class="mp-foster-card">
      <div class="mp-foster-img" onclick="openFosterDetail('${pet.id}')">
        <img src="${pet.image}" alt="${escapeHtml(pet.name)}" onerror="this.style.opacity='0'">
      </div>
      <div class="mp-foster-info">
        <div class="mp-foster-header">
          <p class="mp-foster-name">${escapeHtml(pet.name)}</p>
          <span class="mp-foster-status status-${pet.status}">${status.label}</span>
        </div>
        <p class="mp-foster-meta">${pet.age} · ${escapeHtml(pet.breed)}</p>
        <div class="mp-foster-stats">
          <span><i class="fas fa-eye"></i> ${pet.views}</span>
          <span><i class="fas fa-users"></i> ${pet.applications.length}</span>
          <span><i class="fas fa-heart"></i> $${pet.donationsReceived}</span>
        </div>
      </div>
      <button class="mp-foster-menu" onclick="openFosterMenu('${pet.id}')" aria-label="Options">
        <i class="fas fa-ellipsis-vertical"></i>
      </button>
    </div>
  `;
...
function openFosterMenu(petId) {
  ...
  modal.querySelector('.modal-content').innerHTML = `
    ...
    <h2 class="modal-title"><i class="fas fa-paw"></i> ${escapeHtml(pet.name)}</h2>
    <div style="display:flex; flex-direction:column; gap:8px; margin-top:8px;">
      <button class="btn btn-outline w-full" onclick="closeModal('quickViewModal'); openEditFosterModal('${petId}');">
        <i class="fas fa-pen"></i> Edit Listing
      </button>
      <button class="btn btn-outline w-full" onclick="closeModal('quickViewModal'); viewFosterApplications('${petId}');">
        <i class="fas fa-users"></i> View Applications (${pet.applications.length})
      </button>
      ${pet.status === 'available' ? `
        <button class="btn btn-primary w-full" onclick="closeModal('quickViewModal'); markFosterAsFostered('${petId}');">
          <i class="fas fa-check"></i> Mark as Fostered
        </button>
      ` : ''}
      <button class="btn btn-danger w-full" onclick="closeModal('quickViewModal'); confirmDeleteFoster('${petId}');">
...
/* ============================================================ */
/* MY FOSTER APPLICATIONS (as applicant)                         */
/* ============================================================ */
function renderMyApplicationsSection() {
  if (!isLoggedIn()) return '';

  const myApps = [];
  FOSTER_PETS.forEach(pet => {
    pet.applications.forEach(app => {
      if (app.applicantId === 'user_self') {
        myApps.push({ pet, app });
      }
    });
  });

  if (!myApps.length) return ''; // Don't show section if no applications

  return `
    <section class="mp-section-card" id="mpMyAppsSection">
      <div class="mp-section-header">
        <h3><i class="fas fa-file-alt"></i> My Applications</h3>
        <span class="mp-count-badge">${myApps.length}</span>
      </div>
      <div class="mp-apps-list">
        ${myApps.map(({ pet, app }) => `
          <div class="mp-app-card" onclick="openFosterDetail('${pet.id}')">
            <div class="mp-app-img">
              <img src="${pet.image}" alt="" onerror="this.style.opacity='0'">
            </div>
            <div class="mp-app-info">
              <p class="mp-app-pet">${escapeHtml(pet.name)}</p>
              <p class="mp-app-date">Applied ${timeAgo(app.submittedAt)}</p>
            </div>
            <span class="mp-app-status status-${app.status}">${app.status}</span>
          </div>
        `).join('')}
      </div>
    </section>
...
/* 27. EXPORT                                                    */
/* ============================================================ */
window.renderMyFosterSection = renderMyFosterSection;
window.renderMyFosterCard = renderMyFosterCard;
window.openFosterMenu = openFosterMenu;
window.renderMyApplicationsSection = renderMyApplicationsSection;
