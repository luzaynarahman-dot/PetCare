/* ============================================================ */
/* PETCARE v3.0 — DEMO MODE (Clean v2)                           */
...
const DemoMode = {
  ...
  async load() {
    ...
    try {
      ...
      /* ---------- FOSTER SYSTEM (User's Own Listings) ---------- */
      if (D.myFosterListings && D.myFosterListings.length) {
        // Backup to localStorage for later re-injection
        localStorage.setItem('pc_demo_myFosterListings', JSON.stringify(D.myFosterListings));

        // Immediate inject if FOSTER_PETS is available
        if (typeof FOSTER_PETS !== 'undefined' && Array.isArray(FOSTER_PETS)) {
          D.myFosterListings.forEach(userPet => {
            const exists = FOSTER_PETS.find(p => p.id === userPet.id);
            if (!exists) {
              FOSTER_PETS.unshift(userPet);
            }
          });

          // Add user's applications to existing pets
          if (D.myFosterApplications && D.myFosterApplications.length) {
            D.myFosterApplications.forEach(app => {
              const pet = FOSTER_PETS.find(p => p.id === app.petId);
              if (pet) {
                const exists = pet.applications.find(a => a.id === app.id);
                if (!exists) {
                  pet.applications.push(app);
                }
              }
            });
          }
          ...
          // Save
          if (typeof saveFosterPets === 'function') {
            saveFosterPets();
          }
        }
      }
...
console.log('PetCare Demo loaded — Clean mode ready');