...
function getUserInitial(authorId, fallbackName) {
  ...
  if (authorId === 'user_self') {
    // ⭐ If not logged in — return guest icon marker
...
function renderAvatarInner(authorId, fallbackName, roleClass) {
  ...
  // ⭐ If no avatar — show initial directly
  if (!avatar || avatar === 'null' || avatar === 'undefined') {
    return `<span class="avatar-initial">${initial}</span>`;
  }

  // ⭐ If avatar exists — show image
  return `<img src="${avatar}" alt="" class="avatar-img" onerror="this.style.display='none';this.parentElement.innerHTML='<span class=\\'avatar-initial\\'>${initial}</span>';">`;
}
...
function renderComposerAvatar() {
  ...
  try {
    ...
    } else if (typeof window.APP !== 'undefined' && window.APP.token && window.APP.user) {
      userLoggedIn = true;
    }
  } catch (err) {
    userLoggedIn = false;
  ...
  if (!userLoggedIn) {
    return `
      <div class="memory-composer-avatar guest" style="width:38px;height:38px;min-width:38px;max-width:38px;min-height:38px;max-height:38px;border-radius:50%;overflow:hidden;display:flex;align-items:center;justify-content:center;flex-shrink:0;padding:0;margin:0;box-sizing:border-box;background:linear-gradient(135deg,#E8D9BF,#C9A961);color:#6B5443;">
        <i class="fas fa-user" style="font-size:16px;"></i>
      </div>
    `;
  }

  // Logged in — show avatar or initial
  ...
  if (avatar && avatar !== 'null' && avatar !== 'undefined') {
    return `
      <div class="memory-composer-avatar" style="width:38px;height:38px;min-width:38px;max-width:38px;min-height:38px;max-height:38px;border-radius:50%;overflow:hidden;display:flex;align-items:center;justify-content:center;flex-shrink:0;padding:0;margin:0;box-sizing:border-box;background:linear-gradient(135deg,#FFD966,#FF9B50);">
        <img src="${avatar}" alt="" style="width:38px;height:38px;min-width:38px;max-width:38px;min-height:38px;max-height:38px;object-fit:cover;object-position:center;border-radius:50%;display:block;padding:0;margin:0;" onerror="this.style.display='none';this.parentElement.innerHTML='<span class=\\'avatar-initial\\'>${initial}</span>';">
      </div>
    `;
  }
...
function openMemoryComposer() {
  if (!isLoggedIn()) {
    showToast('Please sign in to share a memory');
